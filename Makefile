...
only-plots: $(wildcard results/plot_*.py)
	./results/plot_experiment.py $(DATADIR)/gemm3.dat "gemm3()" "D += A(BC), square matrices" "N" $(DATADIR)/gemm3.$(PLOTEXT)
	./results/plot_experiment.py $(DATADIR)/gemm3_ab_bc_kernel.dat "gemm3()" "D^T += C^T(B^TA^T), square matrices" "N" $(DATADIR)/gemm3_ab_bc_kernel.$(PLOTEXT)
	./results/plot_experiment.py $(DATADIR)/gemm3_parallel.dat "gemm3()" "D += A(BC), square matrices, 4 cores" "N" $(DATADIR)/gemm3_parallel.$(PLOTEXT)
	./results/plot_multi_shape_experiment.py $(DATADIR)/gemm3_rectangles.dat "gemm3()" "D += A(BC), narrow dimension = 9" $(DATADIR)/gemm3_rectangles.$(PLOTEXT)
