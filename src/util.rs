...
pub fn test_c_eq_a_b<T:Scalar, At:Mat<T>, Bt:Mat<T>, Ct:Mat<T>>( a: &mut At, b: &mut Bt, c: &mut Ct ) -> T {
    ...
    let mut nullw: VoidMat<T> = VoidMat::new();
    w.fill_rand();
    cw.fill_zero();
    bw.fill_zero();
    abw.fill_zero();
    ...
    unsafe {
        ref_gemm.run(b, &mut w, &mut bw, &mut nullw, &ThreadInfo::single_thread() );
        ref_gemm.run(a, &mut bw, &mut abw, &mut nullw, &ThreadInfo::single_thread() );
    }

    //Do cw = Cw
    unsafe {
        ref_gemm.run( c, &mut w, &mut cw, &mut nullw, &ThreadInfo::single_thread() );
    }
