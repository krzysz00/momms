...
The complete algorithm is Algorithm \ref{alg:gemm3}.
...
\begin{algorithm}
  \caption{Algorithm for \gemm3{}}
  \label{alg:gemm3}
  \textbf{TODO picture}
  \begin{algorithmic}
    \Procedure{gemm3}{$A, B, C, D$}
    \For{$j_o \gets 0, n_{Ci}, \ldots$ \TO{} $n$}
    \For{$p_o \gets 0, m_{Ci}, \ldots$ \TO{} $k$}
    \For{$p_i \gets 0, k_{Ci}, \ldots$ \TO{} $l$}
    \State{pack $C[p_i:p_i+k_{Ci},j_o:j_o+n_{Ci}] \to \tilde{B_i}$}
    \For{$i_i \gets p_0, p_0 + m_{Ci2}, \ldots$ \TO{} $p_0 + m_{Ci}$}
    \State{pack $B[i_i:i_i+m_{Ci2},p_i:p_i+k_{Ci}] \to \tilde{A_i}$}
    \State{$\textsc{macrokernel}(\tilde{A_i}, \tilde{B_i}, \tilde{B_o})$}
    \Comment{writes in packed form}
    \EndFor{}
    \EndFor{}
    \For{$i \gets 0, m_{Co}, \ldots$ \TO{} $m$}
    \For{$p' \gets p, p + k_{C2}, \ldots$ \TO{} $p + k_C$}
    \State{pack $A[m:m+m_C,p':p'+k_{C2}] \to \tilde{A_o}$}
    \State{$\textsc{macrokernel}(\tilde{A_o}, \tilde{B_o}, D[i:i+m_C,j:j+n_C])$}
    \EndFor{}
    \EndFor{}
    \EndFor{}
    \EndFor{}
